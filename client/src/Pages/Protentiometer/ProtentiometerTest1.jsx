...
const RainfallChart = () => {
  ...
  useEffect(() => {
    const fetchTemperatureData = async () => {
      try {
        const response = await fetch("https://iinms.brri.gov.bd/api/stats/temperature-test");
...
const SoilMoisture = () => {
  ...
  useEffect(() => {
    const fetchTemperatureData = async () => {
      try {
        const response = await fetch("https://iinms.brri.gov.bd/api/stats/soil-moisture-test");
        const data = await response.json();
        console.log(data);
        setSelectedDate(data.average)
        if (Array.isArray(data.last20)) {
...
const WaterLevel = () => {
  ...
  useEffect(() => {
    const fetchWaterLevelData = async () => {
      try {
        const response = await fetch("https://iinms.brri.gov.bd/api/stats/water-level-test");
        const data = await response.json();
        console.log(data);
        setSelectedDate(data.average)
        if (Array.isArray(data.last20)) {
...
export default Protentiometer1